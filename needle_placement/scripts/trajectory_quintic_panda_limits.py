...
max_acceleration = 25*0.001  # 25.0*0.00008 rad/s² 

# Create your plot figures and lines before the main ROS loop
plt.ion()
...
def move_to_target(t0):
    ...
    # Create figure for joint position trajectories
    plt.figure(1)
    plt.title('Joint Position Trajectories')
    position_lines = [plt.plot([], [])[0] for _ in range(len(joint_names))]

    # Create figure for joint acceleration trajectories
    plt.figure(2)
    plt.title('Joint Acceleration Trajectories')
    acceleration_lines = [plt.plot([], [])[0] for _ in range(len(joint_names))]
    ...
    for t in np.arange(t0, tf, 0.01): # 0.01
        ...
        # Update position and acceleration data
        for i in range(len(joint_names)):
            position_lines[i].set_xdata(np.append(position_lines[i].get_xdata(), t))
            position_lines[i].set_ydata(np.append(position_lines[i].get_ydata(), joint_positions.data[i]))

        # Make sure both axes are updated
        plt.figure(1)
        plt.xlim(0, tf)
        # plt.ylim(min(target_joint_angles + initial_positions), max(target_joint_angles + initial_positions))
        plt.ylim(min(list(target_joint_angles) + list(initial_positions)), max(list(target_joint_angles) + list(initial_positions)))
