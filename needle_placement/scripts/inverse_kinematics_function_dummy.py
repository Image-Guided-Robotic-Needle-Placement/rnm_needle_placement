...
def initialize_jit_functions():
    dummy_q = np.zeros((7, 1))
    dummy_A = np.zeros((12, 1))
    A_lambdified_dummy = jit(lambdify((q1, q2, q3, q4, q5, q6, q7), A, 'numpy'), nopython=True)
    J_lambdified_dummy = jit(lambdify((q1, q2, q3, q4, q5, q6, q7), J, 'numpy'), nopython=True)
    incremental_ik(dummy_q, dummy_A, dummy_A, A_lambdified_dummy, J_lambdified_dummy)
