...
'''Author: Selvakumar Nachimuthu
   This node saves the current pointcloud as pcd file. Also, it saves the corresponding poses in .npy file
'''
...
class PointCloudConverterNode:
    def __init__(self):
        ...
        #subscribers
        rospy.Subscriber('/k4a/points2', PointCloud2, self.pointcloud_callback, queue_size=2) 
        rospy.Subscriber('joint_states', JointState, self.jointstate_callback)
        rospy.Subscriber('/trajectory_done', Bool, self.trajectory_done_callback)
        ...
        self.save_directory = '/home/selva/Desktop/presentation/jointstates'
        self.pointcloud_save_directory = '/home/selva/Desktop/presentation/pointclouds'
        ...
        rosparam.set_param('model_recording_done', 'False')
    ...
    def save_pointcloud(self, msg):
        ...
        if self.is_trajectory_done == True:
            ...
            self.pointcloud_count += 1
            print('self.pointcloud', self.pointcloud_count)
    ...
    def save_joint_state(self):
        ...
        if self.is_trajectory_done and hasattr(self, 'latest_joint_state'):
            ...
            if self.pointcloud_count == 1:    #if enough pointclouds are stored it converts the jointstates into poses and stores them as npy file 
                self.combine_joint_states()
                self.convert_to_poses()
                rosparam.set_param('model_recording_done', 'True')  #sets the rosparam if recording of pcd's and jointstates are done (used in bash script)
                print('model_recording_done')
                rospy.loginfo('Finished processing all point clouds')
