...
class BagRecorderNode:
    def __init__(self):
        ...
        self.bag_filename = "camera_calibration.bag"
    ...
    def reached_position_callback(self, data):
        if data.data:
            ...
            if os.path.isfile(self.bag_filename):
                print('appending..')
                bag = rosbag.Bag(self.bag_filename, "a")
            else:
                print('writing..')
                bag = rosbag.Bag(self.bag_filename, "w")
            ...
            depth_msg = rospy.wait_for_message("/k4a/depth/image_raw", Image, timeout=rospy.Duration(1))
            ...
            bag.write("/k4a/rgb/image_raw", rgb_msg)
            bag.write("/k4a/ir/image_raw", ir_msg)
            bag.write("/k4a/depth/image_raw", depth_msg)
            bag.write('/franka_state_controller/joint_states_desired', joint_msg)
