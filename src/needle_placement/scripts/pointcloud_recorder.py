...
class PointCloudRecorderNode:
    ...
    def reached_position_callback(self, data):
        ...
        if data.data == True:
            ...
            point_clouds = rospy.wait_for_message("/k4a/points2", PointCloud2, timeout=rospy.Duration(20))
            joint_msg = rospy.wait_for_message('/franka_state_controller/joint_states_desired', JointState, timeout=rospy.Duration(5))

            # Save the joint states
            joint_msg = np.array(joint_msg.position)
            jointstate_array = joint_msg.flatten()
            f = open("/home/rnm-group2/group2/rnm_needle_placement/src/needle_placement/lab/pointclouds/jointstates.txt", "a")
            ...
            o3d.io.write_point_cloud("/home/rnm-group2/group2/rnm_needle_placement/src/needle_placement/lab/pointclouds/test.pcd",out_pcd)

            rospy.loginfo("Besma said I have to change this to pointcloud acquired")
